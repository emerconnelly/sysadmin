...
if (Get-ItemProperty -Path "HKCU:\SOFTWARE\Palo Alto Networks\GlobalProtect\Settings" -Name "LastUrl") {
    # if LastUrl is not pointing to $new_portal then non-compliant
    $key_value = Get-ItemProperty -Path "HKCU:\SOFTWARE\Palo Alto Networks\GlobalProtect\Settings" -Name "LastUrl"
